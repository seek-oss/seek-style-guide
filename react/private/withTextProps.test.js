...
describe('withTextProps:', () => {
  ...
  it('should throw prop validation error if size is unknown', () => {
    ...
    const expectedError = expect.stringMatching(/Invalid prop size='head' supplied to OriginalComponent/);
  ...
  it('should omit size prop if boolean size is supplied and throw prop validation error', () => {
    ...
    const expectedError = expect.stringMatching(/Seems that you've accidentially supplied boolean size along with size='heading' to OriginalComponent, please remove one of them. Otherwise boolean prop will overwrite the 'size' prop/);
    expect(spy).toBeCalledWith(expectedError);
  });
